    ...
    public void onUpdateEventReceived(Update update) {//reacciona a una interaccion
        ...
        if (getMessageText().equals("/start")){// equals = a =
            ...
            sendTextMessageAsync(STEP_1_TEXT,
                    Map.of("Hackear la nevera", "step_1_btn"));//Metodo tipo de objeto guarda informacion key and value
        }
        ...
        if (getCallbackQueryButtonKey().equals("step_2_btn")){
            addUserGlory(20);
            sendTextMessageAsync(STEP_3_TEXT+ getUserGlory(),
                    Map.of("Hackear al robot aspirador","step_3_btn"));
        }
        if (getCallbackQueryButtonKey().equals("step_3_btn")){
            addUserGlory(30);
            sendTextMessageAsync(STEP_4_TEXT+ getUserGlory(),
        ...
        if (getCallbackQueryButtonKey().equals("step_4_btn")){
            addUserGlory(30);
            sendTextMessageAsync(STEP_5_TEXT+ getUserGlory(),
                    Map.of("Encender y ponerse la GoPro!","step_5_btn"));
        }
        if (getCallbackQueryButtonKey().equals("step_5_btn")){
            addUserGlory(40);
            sendTextMessageAsync(STEP_6_TEXT+ getUserGlory(),
        ...
        if (getCallbackQueryButtonKey().equals("step_6_btn")){
            addUserGlory(40);
            sendTextMessageAsync(STEP_7_TEXT+ getUserGlory(),
                    Map.of("Romper la contraseña","step_7_btn"));
        }
        if (getCallbackQueryButtonKey().equals("step_7_btn")){
            addUserGlory(50);
            sendTextMessageAsync(STEP_8_TEXT+ getUserGlory(),
                    Map.of("Salir al patio","step_8_btn"));
        }
        if (getCallbackQueryButtonKey().equals("step_8_btn")){
            addUserGlory(0);
            sendTextMessageAsync(FINAL_TEXT+ getUserGlory());
        }
    }
